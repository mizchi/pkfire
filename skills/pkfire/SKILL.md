...
jobs:
  ci:
    ...
    steps:
      ...
      - uses: mizchi/pkfire@v0.6.0      # or @v0 to track latest 0.x
      - run: pkf run ci
```
...
**Do not write `uses: mizchi/pkfire@pkfire@0.6.0`.** GHA's parser
...
| Input | Default | Notes |
| --- | --- | --- |
| `version` | inferred from `${{ github.action_ref }}`, falls back to latest release | Accepts `v0.5.0`, `0.4.0`, `v0` (floating major), or the underlying `pkfire@0.6.0`. Pinning via `uses: mizchi/pkfire@v0.6.0` is the recommended form. |
